 ...
 * @author shaddie
 */
public class PrintCurrentStockReport {
    ...
    public static void generateReport(TableView table_to_export ,String path,String title){
       ...
       String columnNames[] = {"ID", "SKU Name","In Stock",  "Reorder Level","Deviation","Reorder Required?"};
   ...
   PdfPCell cell;
    ...
    for (int i = 0; i < columns; i++) {
     table.addCell(new Phrase(columnNames[i], FontFactory.getFont("Helvetica", 12.0F,Font.BOLD)));
          ...
          CurrentStockReport record ;
    for (Object dailySales : list) {
    record = (CurrentStockReport)dailySales;
table.addCell(new Phrase(""+record.getSkuid(), FontFactory.getFont("Helvetica", 11.0F)));
 ...
 table.addCell(new Phrase(""+record.getDiff(), FontFactory.getFont("Helvetica", 11.0F)));
 table.addCell(new Phrase(""+record.getReorderLevel(), FontFactory.getFont("Helvetica", 11.0F)));
 table.addCell(new Phrase(""+record.getDeviation(), FontFactory.getFont("Helvetica", 11.0F)));
 table.addCell(new Phrase(""+record.getPurchaseRequired(), FontFactory.getFont("Helvetica", 11.0F)));
    }
      document.add(table);
       document.add(new Phrase("\n"));
